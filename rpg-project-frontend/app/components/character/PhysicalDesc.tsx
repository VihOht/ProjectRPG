...
interface CharacterPhysicaldescProps {
    physicaldesc: string;
    handlePhysicaldescChange: (value: string) => void;
    update: () => void;
}

export function CharacterPhysicaldesc({
    physicaldesc,
    handlePhysicaldescChange,
    update,
}: CharacterPhysicaldescProps) {
        ...
        <section className="mb-8">
            ...
            {isEditing ? (
                ...
                <p className="text-vaccineGray-300 whitespace-pre-line">
                    {physicaldesc || "Nenhuma descrição adicionada."}
                </p>
            )}
        </section>
    );
